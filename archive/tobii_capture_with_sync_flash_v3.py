...
from obs_controller import ObsConfig, ObsController

IS_WINDOWS = platform.system() == "Windows"

SCRIPT_VERSION = "4.0.0-obs-reqclient"
...
def parse_args():
    ...
    p.add_argument("--obs-host", default=None,
                    help=f"OBS接続先ホスト(既定: {obs_controller.DEFAULT_OBS_HOST})")
    p.add_argument("--obs-port", type=int, default=None,
                    help=f"OBS接続先ポート(既定: {obs_controller.DEFAULT_OBS_PORT})")
    ...
    p.add_argument("--obs-timeout-sec", type=float, default=None,
                    help=f"OBS応答/確認イベント待ちタイムアウト秒"
                         f"(既定: {obs_controller.DEFAULT_OBS_TIMEOUT_SEC})")
...
def main():
    ...
    try:
        ...
        # --- OBS録画開始要求 (F-4 手順5-6, F-2, F-7) ---
        # 要件定義書F-4の指定順序どおり、Tobii購読開始の「後」にOBS開始要求を送る。
        # これは同時開始そのものを狙うのではなく、各段階のUTC時刻を個別に記録して
        # 後処理で差分比較できるようにするための順序である。
        if obs_ctrl is not None:
        ...
        if session_aborted_due_to_obs:
            log_event("aborted_after_start_due_to_obs_failure")
        else:
    ...
    metadata = {
        ...
        "known_limitations": [
            ...
            "OBS録画自動制御(mode='automatic')の場合、要件定義書F-4の順序に従い"
            "Tobii購読開始の後にOBS録画開始要求を送信している。そのため、OBSが"
            "録画開始要求からエンコーダが実際に書き込みを開始するまでの間、"
            "セッション冒頭のごく短い期間の映像が欠落する可能性が理論上ある。"
            "これは要求送信時刻(obs_start_request_utc)と開始確認時刻"
            "(obs_record_started_utc)の差分から後処理で検出・評価できる。",
        ],
    }
