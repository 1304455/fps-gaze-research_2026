...
【v2 (start_sync) での変更点】
    VALORANT + Vanguard 環境では映像上に描画マーカー(SyncFlashWindow)を
    載せられないため、開始同期の主たる評価手段を「視覚フラッシュ」から
    「ログ・メタデータのみによる時刻比較」に変更した。
      - 基準点: Tobiiの最初の視線サンプル到達時刻
        (GazeRecorder.first_sample_info['wall_timestamp_utc'])
      - 比較対象: OBS録画開始要求時刻 (obs_start_request_utc) および
        可能であれば実開始確認時刻 (obs_record_started_utc)
    この差分とディスプレイのリフレッシュレートから「1フレーム未満か」を
    判定する。計算ロジックは本モジュール内の compute_start_sync_metrics()
    に集約し、呼び出し側(main側)・分析側の双方から同じ定義で参照できる
    ようにしている。SyncFlashWindowは検証用として引き続き利用できるが、
    本番実験(VALORANT)での同期の主手段ではない。
...
_SRC_DIR = _THIS_FILE.parent if _THIS_FILE.parent.name == "src" else None
if _SRC_DIR is None:
...
if _SRC_DIR and str(_SRC_DIR.parent) not in sys.path:
...
class ObsSyncRecord:
    ...
    # ---- 開始同期 (start_sync) ------------------------------------------
    # フラッシュマーカーに依存しない、ログ・メタデータのみでの開始同期評価用。
    # 値の算出は compute_start_sync_metrics() に一元化し、ここには結果のみを
    # 保持する(jsonl同期ログ・metadata JSONの双方から同じ値を参照するため)。
    start_sync_method: Optional[str] = None
    gaze_first_sample_utc: Optional[str] = None
    start_sync_delta_sec: Optional[float] = None
    start_sync_delta_frames: Optional[float] = None
    start_sync_delta_sec_confirmed: Optional[float] = None
    start_sync_delta_frames_confirmed: Optional[float] = None
    start_sync_within_one_frame: Optional[bool] = None
...
# 開始同期評価 (start_sync)
#
# フラッシュマーカーではなく、Tobiiの最初の視線サンプル時刻とOBS側の時刻を
# 比較することで開始同期精度を評価するための共通ロジック。main側・本モジュール
# 側の両方（および将来の分析スクリプト）から同一の定義で呼び出せるよう、
# ここに一元化する。
# ============================================================================

def compute_start_sync_metrics(
    gaze_first_sample_utc: "Optional[str]",
    obs_start_request_utc: "Optional[str]",
    obs_record_started_utc: "Optional[str]",
    refresh_rate_hz: "Optional[float]",
) -> dict:
    """Tobii最初のサンプル時刻を基準に、OBS側時刻との差分を算出する。

    2種類の差分を併記する(卒論等での考察を意識した設計):

      - delta_sec_request_based / delta_frames_request_based:
          「OBSへ録画開始を要求した時刻(obs_start_request_utc)」との差分。
          要求送信〜実際にエンコーダが書き込みを始めるまでのOBS内部の
          遅延は含まれないため、実際のズレを「過小評価する可能性がある」
          指標である。
      - delta_sec_confirmed_based / delta_frames_confirmed_based:
          EventClientで実際に受信した「録画開始確認(RecordStateChanged=
          STARTED)」時刻(obs_record_started_utc)との差分。要求送信〜
          OBS内部処理〜WebSocketイベント配送までを含むため、
          request_based よりズレを「過大評価する可能性がある」指標である。
          EventClient未接続、または確認イベントを規定時間内に受信できな
          かった場合は None になる。

    真の「映像1フレーム目の提示時刻」は、原理的にはこの2指標の間のどこかに
    あると考えるのが妥当であり、両者を挟み込みとして報告することで、
    フラッシュマーカーなしでも定量的な誤差の上限・下限を示せるようにする。

    delta = t_obs - t_gaze。正の値はOBS側(の該当イベント)がTobiiの最初の
    サンプルより「後」であることを意味する。
    """
    ...
    t_gaze = _parse(gaze_first_sample_utc)
    t_obs_request = _parse(obs_start_request_utc)
    t_obs_confirmed = _parse(obs_record_started_utc)

    result = {
        "gaze_first_sample_utc": gaze_first_sample_utc,
        "obs_start_request_utc": obs_start_request_utc,
        "obs_record_started_utc": obs_record_started_utc,
        "refresh_rate_hz": refresh_rate_hz,
        "delta_sec_request_based": None,
        "delta_frames_request_based": None,
        "within_one_frame_request_based": None,
        "delta_sec_confirmed_based": None,
        "delta_frames_confirmed_based": None,
        "within_one_frame_confirmed_based": None,
    }

    if t_gaze is not None and t_obs_request is not None:
        delta = (t_obs_request - t_gaze).total_seconds()
        result["delta_sec_request_based"] = round(delta, 6)
        if refresh_rate_hz:
            frames = delta * refresh_rate_hz
            result["delta_frames_request_based"] = round(frames, 4)
            result["within_one_frame_request_based"] = abs(frames) < 1.0

    if t_gaze is not None and t_obs_confirmed is not None:
        delta_c = (t_obs_confirmed - t_gaze).total_seconds()
        result["delta_sec_confirmed_based"] = round(delta_c, 6)
        if refresh_rate_hz:
            frames_c = delta_c * refresh_rate_hz
            result["delta_frames_confirmed_based"] = round(frames_c, 4)
            result["within_one_frame_confirmed_based"] = abs(frames_c) < 1.0
