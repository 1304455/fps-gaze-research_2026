...
def cmd_analyze(args) -> None:
    ...
    # 定義: video_sec=0(動画ファイル先頭)の実時刻は
    #   gaze_first_sample_utc + offset  (offset = t_plus - video_sec)
    # に相当する(T+はgaze_first_sampleからの経過秒であり、動画は実時間に
    # 対して線形に進むと仮定しているため)。
    ...
    print(f"gaze_first_sample_utc: {gaze_first_sample_utc}")
    print(f"読み取り点 (video_sec, T+):        `{points}`")
    print(f"点ごとの推定オフセット(秒):        `{[round(x, 4) for x in implied_offsets]}`")
    if sd_offset is not None:
        print(f"平均オフセット: {mean_offset:.4f} 秒 (SD={sd_offset:.4f} 秒, n={len(points)})")
    else:
        print(f"平均オフセット: {mean_offset:.4f} 秒 (1点のみのためSD計算不可)")

    print()
    print(f"参考(同一セッションのログベース推定値):")
    print(f"  delta_sec_request_based   = {request_based}")
    print(f"  delta_sec_confirmed_based = {confirmed_based}")
    ...
    if len(points) >= 2:
        ...
        if var_x > 0:
            ...
            print()
            print(f"回帰による傾き(理想値=1.0): {slope:.6f}")
            print(
            ...
            print("\n(全ての読み取り点のvideo_secが同一のため、傾きは計算できません)")
